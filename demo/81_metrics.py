    ...
    for filename in metrics_files:
        ...
        try:
            with fits.open(filename) as hdul:
                ...
                # Find dynamic HDUs
                auto_power_ant_hdus = [hdu.name for hdu in hdul if hdu.name.startswith('AUTO_POWER_ANT=')]
                auto_sub_ant_hdus = [hdu.name for hdu in hdul if hasattr(hdu, 'header') and 'ANT_ID' in hdu.header]
                auto_delay_pol_hdus = [hdu.name for hdu in hdul if hdu.name.startswith('AUTO_DELAY_POL=')]

                missing_hdus = [hdu_name for hdu_name in required_hdus if hdu_name not in [h.name for h in hdul]]
                if missing_hdus:
                    continue
                ...
                for hdu_name in additional_hdus:
                    try:
                        ...
                        if hdu_name.startswith('AUTO_POL='):
                            ...
                            nan_array_2d = np.full((128, ao_flag_aligned.shape[1]), np.nan)
                    ...
                    try:
                        ...
                        if ant_id is not None:
                            ...
                            if ant_id not in antenna_table:
                                ...
                                key_fields = ['ANTNAME', 'ANT_ID', 'ANT_NUM', 'ANT_TYPE', 'RX_NUMBER', 'RX_SLOT', 'RX_TYPE']
...
def plot_waterfall(data, name, show=False, save=True):
    ...
    for shape_key in all_data_2d:
        ...
        shape_times = np.arange(shape_time_start, shape_time_end + shape_time_step, shape_time_step)
        ...
        for col_name in all_data_2d[shape_key]:
            ...
            if all_data_2d[shape_key][col_name]:
                ...
                for data_2d, times_2d in zip(all_data_2d[shape_key][col_name], all_times_2d[shape_key][col_name]):
                    for i, gps_time in enumerate(times_2d):
                        time_idx = int(round((gps_time - shape_time_start) / shape_time_step))
        ...
        fig2, axes2 = plt.subplots(1, 7, figsize=(28, 32), sharex=True, sharey=True, tight_layout=True, dpi=100)
        ...
        for i, (col_name, color) in enumerate(zip(column_names, colors)):
            if col_name in waterfall_data and len(waterfall_data[col_name]) > 0:
                ...
                plt.colorbar(im, ax=axes2[i], orientation="horizontal", pad=0.1, shrink=0.8)
    ...
    for rx_num in sorted(receivers.keys()):
        ...
        for pol_name in ["XX", "YY", "XY"]:
            ...
            if hdu_name in additional_data_2d and additional_data_2d[hdu_name]:
                ...
                file_time_grid = np.arange(file_time_start, file_time_end + time_step_seconds, time_step_seconds)
                ...
                for file_idx, file_time in enumerate(file_times):
                    ...
                    for ant_id in ant_ids_in_rx:
                        if ant_id < n_antennas and ant_id in antenna_grids:
                            antenna_grids[ant_id][time_bin_idx, :] = all_files_data[file_idx, ant_id, :]

                fig, axes = plt.subplots(2, 4, figsize=(20, 10), sharex=True, sharey=True)
                fig.suptitle(f"{name} - AUTO_POL {pol_name} - Receiver {rx_num}", fontsize=12, fontweight="bold")
                axes = axes.flatten()
                ...
                for plot_idx, ant_id in enumerate(ant_ids_in_rx):
                    if plot_idx < 8 and ant_id in antenna_grids:
                        ...
                        extent = [freq_mhz_auto[0], freq_mhz_auto[-1], time_gps_grid[0], time_gps_grid[-1]]
                ...
                plt.tight_layout()
                cbar = fig.colorbar(im, ax=axes, orientation="vertical", pad=0.02, shrink=0.8)
...
def plot_amp_fp_grid(data, name, show=False, save=True):
    ...
    amp_fp_base_names = ["EAVILS_MEAN_AMP_FP", "EAVILS_SQRT_MEAN_VAR_AMP_FP", "SSINS_DIFF_MEAN_AMP_FP"]
    ...
    fig4, axes4 = plt.subplots(3, 3, figsize=(24, 12), sharex=True, sharey=True, dpi=150, tight_layout=True)
    ...
    for col_idx, base_name in enumerate(amp_fp_base_names):
        for row_idx, pol_suffix in enumerate(pol_suffixes):
            ...
            if pol_key in additional_data_2d and additional_data_2d[pol_key]:
                ...
                extent = [freq_mhz[0], freq_mhz[-1], time_gps_grid[0], time_gps_grid[-1]]
            ...
            if row_idx == 0:
                axes4[row_idx, col_idx].set_title(base_name.replace("_", " "), fontsize=9, fontweight="bold")
            if col_idx == 0:
                axes4[row_idx, col_idx].set_ylabel(f"{pol_suffix[1:]} Pol\nTime (min)", fontsize=9, fontweight="bold")
            if row_idx == 2:
                axes4[row_idx, col_idx].set_xlabel("Frequency (MHz)", fontsize=9, fontweight="bold")
...
def plot_auto_sub_ant(data, name, show=False, save=True):
    ...
    for ant_id in ant_ids:
        ...
        fig6.suptitle(f'{name} - AUTO_SUB_ANT {antenna_display_name}', fontsize=12, fontweight='bold')
        ...
        if pol_name in auto_delay_pol_data:
            ...
            if delay_file_data:
                ...
                for rx_num in sorted(receivers.keys()):
                    ...
                    fig7.suptitle(f'{name} - AUTO_DELAY_POL {pol_name} - Receiver {rx_num}', fontsize=12, fontweight='bold')
                    axes7 = axes7.flatten()
