...
Examples:
  ...
  %(prog)s --name 1436729760 birli_*_metrics.fits
    ...
    parser.add_argument('--name', required=True,
                        help='Name for the output plots (e.g., observation ID)')
    parser.add_argument('--show', action='store_true',
                        help='Show plots interactively')
    parser.add_argument('files', nargs='+',
                        help='Metrics FITS files to process')
    ...
    for filename in metrics_files:
        ...
        try:
            with fits.open(filename) as hdul:
                ...
                required_hdus = ['AO_FLAG_METRICS', 'SSINS_POL=XX', 'SSINS_POL=XY', 'SSINS_POL=YY',
                                 'EAVILS_POL=XX', 'EAVILS_POL=XY', 'EAVILS_POL=YY']
                ...
                crval1 = header.get('CRVAL1', 167040000.0)  # Frequency of reference pixel (Hz)
                ...
                all_times_2d[shape_key]['SSINS_POL=XX'].append(gps_times)  # No offset for waterfall
    ...
    for i, (col_name, color) in enumerate(zip(column_names, colors)):
        if len(aligned_data[col_name]) > 0:
            axes[i].plot(time_minutes_common, aligned_data[col_name], color=color, alpha=0.7, linewidth=0.5)
            # Set y-axis limits to show variation better
            data_range = np.nanmax(aligned_data[col_name]) - np.nanmin(aligned_data[col_name])
            if data_range > 0:
    ...
    for shape_key in all_data_2d:
        ...
        fig2, axes2 = plt.subplots(1, 7, figsize=(28, 8), sharex=True, sharey=True, tight_layout=True, dpi=100)
        ...
        for i, (col_name, color) in enumerate(zip(column_names, colors)):
            if col_name in waterfall_data and len(waterfall_data[col_name]) > 0:
                ...
                im = axes2[i].imshow(data_2d_norm, aspect='auto', origin='lower',
                                     extent=[freq_mhz[0], freq_mhz[-1], time_minutes_2d[0], time_minutes_2d[-1]],
    ...
    if additional_times:
        ...
        add_common_times = np.arange(add_time_start, add_time_end + time_step, time_step)
            ...
            if additional_data[key]:
                ...
                for i, times_list in enumerate(additional_times):
                    if i < len(additional_data[key]):
                        ...
                        for j, gps_time in enumerate(times_list):
                            if j < len(data_array):
                                time_idx = int(round((gps_time - add_time_start) / time_step))
        ...
        for hdu_name in ['AUTO_POL=XX', 'AUTO_POL=YY', 'AUTO_POL=XY']:
            if additional_data_2d[hdu_name]:
                ...
                file_time_grid = np.arange(
                    file_time_start, file_time_end + time_step_seconds, time_step_seconds)
                ...
                for file_idx, file_time in enumerate(file_times):
                    ...
                    for ant_idx in range(max_antennas_to_plot):
                        if ant_idx < n_antennas:
                            antenna_grids[ant_idx][time_bin_idx, :] = all_files_data[file_idx, ant_idx, :]

                # Create subplots for multiple antennas
                fig, axes = plt.subplots(3, 4, figsize=(20, 12), sharex=True, sharey=True)
                fig.suptitle(f'{name} - {hdu_name} by Antenna', fontsize=10, fontweight='bold')
                axes = axes.flatten()
                ...
                for ant_idx in range(max_antennas_to_plot):
                    if ant_idx < n_antennas:
                        ...
                        extent = [freq_mhz_auto[0], freq_mhz_auto[-1],
                                  time_minutes_grid[0], time_minutes_grid[-1]]
                        im = axes[ant_idx].imshow(antenna_data, aspect='auto',
                                                  origin='lower', extent=extent,
                                                  cmap='viridis', interpolation='nearest',
                                                  vmin=vmin, vmax=vmax)

                        # Compress the y-axis to reduce vertical space, expand frequency axis
                        axes[ant_idx].set_aspect(aspect=0.3)  # Make it much wider than tall
                ...
                auto_pol_plot_filename = f'auto_pol_antennas_{hdu_name.replace("=", "_")}_{name}.png'
        ...
        for col_idx, base_name in enumerate(amp_fp_base_names):
            for row_idx, pol_suffix in enumerate(pol_suffixes):
                ...
                if pol_key in additional_data_2d and additional_data_2d[pol_key]:
                    ...
                    spectra_list = additional_data_2d[pol_key]  # List of (n_freq,) arrays
                    ...
                    im = axes4[row_idx, col_idx].imshow(
                        spectra_grid, aspect='auto', origin='lower', extent=extent,
                        cmap='viridis', interpolation='nearest', vmin=vmin, vmax=vmax)
