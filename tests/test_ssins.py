...
Longer term should also afford baselining for peak memory usage,
which is the core metric.
...
args = Namespace(
    files=[
        '/Users/raf/source/repos/mwa-demo/demo/data//1061312152/raw/1061312152.metafits',
...
def define_memory_limit(files: list[str] | list[Path],
    ...
    suffix: str = '.fits') -> float:
       """Defines the memory limit as a function of the size of the initial files
       to ensure that if the files were max_size, they would not use more than
       max memory"""
       total_size_bytes = sum(Path(f).stat().st_size for f in files
           if Path(f).exists() and Path(f).suffix == suffix)
       return factor * total_size_bytes / (1024 * 1024) # MB
...
def test_run_ssins_select(ssins_module, diff: bool):
    ...
    peak_memory = memory_usage(call_ssins_read_select, interval=0.01,  max_usage=True) # type: ignore
    assert peak_memory < MEMORY_LIMIT, (
        f"{peak_memory = }MB exceeded {MEMORY_LIMIT = }MB"
    )
